...
class DownBoundary(SubDomain):
    def inside(self, x, on_boundary):
        tol = 1E-16   # tolerance for coordinate comparisons        
        return on_boundary and ((x[1]) < 2)
Gamma_D = DownBoundary()
...
class Elastic:
    def __init__(self, mesh, Vh, Prior):
        ...
        self.atrue = Expression('(15.0 - 5.0*sin(3.1416*(x[1]/20.0 - 0.5)))')
        self.f = Expression(("0.0","0.0022","0.0")) 
        self.u_o = Vector()
        
        self.u_bdr = Expression(("0.0","0.0","0.0")) 
        self.u_bdr0 = Expression(("0.0","0.0","0.0")) 
        self.bc = DirichletBC(self.Vh[STATE], self.u_bdr, Gamma_D)
        self.bc0 = DirichletBC(self.Vh[STATE], self.u_bdr0, Gamma_D)
    ...
    def assembleC(self, x):
        ...
        Cvarf = inner(sigma(s,trial), strain(test))*dx 
        C = assemble(Cvarf)
#        print "||c||", x[PARAMETER].norm("l2"), "||s||", x[STATE].norm("l2"), "||C||", C.norm("linf")
    ...
    def assembleWau(self, x):
        ...
        c = Function(self.Vh[PARAMETER], x[PARAMETER])
        varf = inner(sigma(trial,test), strain(a))*dx 
        Wau = assemble(varf)
        Wau_t = Transpose(Wau)
        self.bc0.zero(Wau_t)
        Wau = Transpose(Wau_t)
    ...
    def assembleRaa(self, x):
        ...
        varf = inner(nabla_grad(trial),nabla_grad(test))*dx 
...
if __name__ == "__main__":
    ...
    Prior = BiLaplacianPrior(Vh[PARAMETER], gamma=10e0, delta=1e-1) #gamma=1e-6, delta=1e-7)
    model = Elastic(mesh, Vh, Prior)

#    print sep, "Test the gradient and the Hessian of the model", sep        
 #   a0 = interpolate(Expression("x[1]+2"), Vh[PARAMETER]) 
 #   modelVerify(model, a0.vector(), 1e-12)
    
    print sep, "Find the MAP point", sep
    a0 = interpolate(Expression("8"),Vh[PARAMETER])
    ...
    atrue = interpolate(Expression('(15.0 - 5.0*sin(3.1416*(x[1]/20.0 - 0.5)))'),Vh[PARAMETER])
